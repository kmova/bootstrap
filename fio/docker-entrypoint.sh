...
if [ "$1" = 'fio' ]; then
    ...
    EXT_WRITE_BW__04_VAL=$(echo "$EXT_WRITE_BW_04"|grep -E 'write:'|grep -Eoi 'BW=[0-9GMKiBs/.]+'|cut -d'=' -f2)
    echo
    echo
    ...
    echo "$EXT_WRITE_BW_16"
    EXT_WRITE_BW__08_VAL=$(echo "$EXT_WRITE_BW_08"|grep -E 'write:'|grep -Eoi 'BW=[0-9GMKiBs/.]+'|cut -d'=' -f2)
    echo
    echo
    ...
    echo "$EXT_WRITE_BW_16"
    EXT_WRITE_BW__16_VAL=$(echo "$EXT_WRITE_BW_16"|grep -E 'write:'|grep -Eoi 'BW=[0-9GMKiBs/.]+'|cut -d'=' -f2)
    echo
    echo
