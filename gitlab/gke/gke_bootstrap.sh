...
function bootstrap(){
  ...
  if ${USE_STATIC_IP}; then
    echo "\n#####"
    echo "Get your external IP using:
    echo "  gcloud compute addresses describe $external_ip_name --region $REGION --project $PROJECT --format='value(address)'"
    echo "You need to add an A record to the DNS name to point to this address.
    echo "See https://gitlab.com/gitlab-org/charts/gitlab/blob/master/doc/installation/cloud/gke.md#dns-entry.";
    echo "#####\n"
  fi
